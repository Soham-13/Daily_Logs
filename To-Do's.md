1. ~~add caveats for csol category in pdf type = 3 and pdf type = 4~~
2. ~~pdf type = 4 code implementation for 186 accessing authorites pdf.~~ 
3. compare json and change detection api and notify the mara agent and occusearch users.  in aura-knowledge-hub.
4. understanding Occusearch SMA table from nikunj bhai.
5. get which visatype are included for whihc category in the json. 
6. add cavet in the json if available in the pdf. 
7. also implement the change detection for visatype as well. 
8. revupro related discussion with utkarsh sir. 